...
enum DATA_FRAME_SUB_TYPE
{
	...
	DataFrameSubType_VehicleInfo,							//车辆信息
	DataFrameSubType_StorageV4_IndexInfo,		//StorageV4的索引信息
	DataFrameSubType_FishEyeHardAdjust,			//鱼眼设备(硬校正)
	DataFrameSubType_FishEyeSoftAdjust,			//鱼眼镜头参数（软校正）
	DataFrameSubType_DistortAdjust,						//畸变校正参数
	DataFrameSubType_VirtualIFrameMask,				//虚拟I帧标识
	DataFrameSubType_VideoAnalyseRule,				//智能分析规则
	DataFrameSubType_VideoAnalyseAlarm,			//智能分析报警
	DataFrameSubType_FaceAreaInfo,						//人脸位置信息
...
typedef struct FaceAreaInfo
{
	...
	unsigned char areaNum; //人脸区域个数
	AreaRect* pAreaRect;	//人脸区域具体数据
}FaceAreaInfo;

// 回调函数类型
typedef void (CALLBACK *AudioCaptureCallBack)(LPBYTE pDataBuffer, DWORD DataLength, DWUSER nUser);
typedef void (CALLBACK *DrawCallBack)(LONG nPort,HDC hDc,DWUSER nUser);
typedef void (CALLBACK *DecodeCallBack)(LONG nPort, LPCSTR pBuf,LONG nSize,FRAME_INFO * pFrameInfo, LONG nPlayedFrames);
typedef void (CALLBACK *DisplayCallBack)(LONG nPort,LPCSTR pBuf,LONG nSize,LONG nWidth,LONG nHeight, LONG nStamp,LONG nType,DWUSER nUser);
typedef void (CALLBACK *FileCallBack)(LONG nPort, DWUSER nUser);
typedef void (CALLBACK *VerifyCallBack)(LONG nPort, FRAME_POS * pFilePos, DWORD bIsVideo, DWUSER nUser);
typedef void (CALLBACK *AudioCallBack)(LONG nPort, PBYTE pAudioBuf, LONG nSize, LONG nStamp, LONG nType, DWUSER nUser);
typedef void (CALLBACK *DemuxCallBack)(LONG nPort, VIDEO_FRAME_INFO *pFrameInfo,DWUSER nUser);
typedef void (CALLBACK *SourceBufCallBack)(LONG nPort,DWORD nBufSize, DWUSER nUser);
typedef void (CALLBACK *ImageSizeChangeCallBack)(LONG nPort, DWUSER nUser);

typedef void (CALLBACK *CoverFileCallBack) (DWORD CurrentPos, DWORD TotoalPos, DWUSER dwUser);//转换进度

//信息帧回调
typedef void (CALLBACK *InfoFramCallBack)(LONG nPort, LONG nType, LPCSTR pBuf,LONG nSize, DWUSER nUser);

//信息帧回调V2类型
typedef void (CALLBACK *InfoFramCallBack_V2)(LONG nPort, LONG nType,LONG nSubType, LPCSTR pBuf,LONG nSize, DWUSER nUser);
...
H264_PLAY_API BOOL 	CALLMETHOD 	H264_PLAY_SetFileEndCallBack(LONG nPort, FileCallBack pProc,DWUSER nUser);
...
H264_PLAY_API BOOL 	CALLMETHOD 	H264_PLAY_SetDisplayCallBack(LONG nPort, DisplayCallBack pProc, DWUSER nUser);
...
//H264_PLAY_RefreshPlay已不使用，程序内部自动刷新
...
H264_PLAY_API BOOL 	CALLMETHOD 	H264_PLAY_SetSourceBufCallBack(LONG nPort,DWORD nThreShold, SourceBufCallBack pProc, DWUSER nUser);
...
H264_PLAY_API BOOL 	CALLMETHOD 	H264_PLAY_SetFileRefCallBack(LONG nPort, FileCallBack pProc, DWUSER nUser);
...
//H264_PLAY_RefreshPlayEx已不使用，程序内部自动刷新
...
H264_PLAY_API BOOL 	CALLMETHOD 	H264_PLAY_RigisterDrawFun(LONG nPort, DrawCallBack pProc,DWUSER nUser);
...
H264_PLAY_API BOOL 	CALLMETHOD 	H264_PLAY_SetVerifyCallBack(LONG nPort, DWORD nBeginTime, DWORD nEndTime, VerifyCallBack pProc, DWUSER nUser);
H264_PLAY_API BOOL 	CALLMETHOD 	H264_PLAY_SetAudioCallBack(LONG nPort, AudioCallBack pProc, DWUSER nUser);
H264_PLAY_API BOOL 	CALLMETHOD 	H264_PLAY_SetImageSizeChangeCallBack(LONG nPort,ImageSizeChangeCallBack pProc,DWUSER nUser);
...
H264_PLAY_API BOOL 	CALLMETHOD 	H264_PLAY_SetDemuxCallBack(LONG nPort, DemuxCallBack pProc, DWUSER nUser);

H264_PLAY_API BOOL 	CALLMETHOD 	H264_PLAY_StartAudioCapture(AudioCaptureCallBack pProc, LONG nBitsPerSample, LONG nSamplesPerSec, LONG nLength, DWUSER nUser);
...
//H264录像文件转Avi录像文件
H264_PLAY_API int CALLMETHOD H264_PLAY_ConvertFile(const char* pSourceFile,const char* pDestFile,  LONG nType, CoverFileCallBack coverPosCallback, DWUSER dwUser);

//信息帧回调
H264_PLAY_API BOOL 	CALLMETHOD 	H264_PLAY_SetInfoFrameCallBack(LONG nPort, InfoFramCallBack pProc, DWUSER nUser);
...
//信息帧回调V2
H264_PLAY_API BOOL 	CALLMETHOD 	H264_PLAY_SetInfoFrameCallBack_V2(LONG nPort, InfoFramCallBack_V2 pProc, DWUSER nUser);
...
{
	int bOrgRate;//0是铺满整个窗口，1按原始比例显示
}H264_PLAY_ORG_RATE;
...
//设置播放参数，当iType为PLAY_PARAM_OrgRate，pParam传入结构体H264_PLAY_ORG_RATE
