...
enum SDK_AudioEncodeTypes
{
	...
	SDK_AUDIO_ENCODE_AAC_8KBIT = 16,
	SDK_AUDIO_ENCODE_AAC_44100Hz = 17,
...
typedef enum _SDK_CONFIG_TYPE
{
	...
	E_SDK_CFG_B_PLAT,						//B协议对接平台--SDK_XM_firm_enable_b_interface_t
	E_SDK_CFG_B_SIM_INFO,					//B 平台sim卡信息--SDK_XM_firm_sim_info_t
	E_SDK_CFG_B_BATTERY_INFO,				//B平台电力信息--SDK_XM_All_battery_info_t
	E_SDK_FILE_IMPORT,						//文件导入设备--SDK_ImportFileHead
}SDK_CONFIG_TYPE;
...
{
	AUTHPRO_CHAP=0,                /* CHAP 认证*/
	AUTHPRO_PAP=1,                /* PAP 认证 */
	AUTHPRO_MAX   
};
...
enum SDK_AlarmFucntionTypes
{
	...
	SDK_ALARM_FUNCTION_TYPE_FUNC_LIGHTCONTRL,		///报警灯显示控制
...
enum SDK_NetServerTypes
{
	...
	SDK_NET_SERVER_TYPES_GA1400,			//GA1400平台
...
enum SDK_OtherFunction
{
	...
	SDK_OTHER_SUPPORT_ENCODE_LOCK,			//锁定编码配置
	SDK_OTHER_HIDE_NORMAL_DAYLIGHTMODE,		//隐藏常规的3个日夜模式选项,并修改智能警戒等名称显示
	SDK_OTHER_SUPPORT_EXT_PLAYBACK,			//支持辅码流回放
	SDK_OTHER_SUPPORT_EXPORT_IPC_LOG,			//支持导出IPC日志(导入导出IPC配置 以及 导出IPC日志 使用同一能力)
	SDK_OTHER_SUPPORT_SHOW_IPC_PARAM,			//支持显示IPC参数
	SDK_OTHER_SUPPORT_MULTI_TRANS_DATA,			//支持多次传输数据(大于64k)
	SDK_OTHER_SUPPORT_LOCAL_CUSTOM_ALARMVIDEO,	//支持本地端自定义报警音
	SDK_OTHER_NR
};
...
typedef struct SDK_CameraAbility
{
	...
	char isSupportSetLDC;	//是否支持畸变校正参数设置，由VPSS去校正
	short minLDC;			//能设置的畸变校正参数最小值
	short maxLDC;			//能设置的畸变校正参数最大值
	char reserve[24];		//保留，如果增加int型的成员用这里
}SDK_CameraAbility;
...
typedef struct SDK_ASB_NET_VSP_CONFIG
{
    ...
    unsigned short	sCsPort;						//服务器端口
	unsigned short	sUdpPort;						//本地绑定udp端口
...
typedef struct SDK_CameraParamEx
{
	...
	int automaticAdjustment; // 自动调节档次 :  1~5
	int LDCValue; //镜头畸变校正参数，取值范围参考CameraAbility中的minLDC和maxLDC
	int res[52];	//冗余 
}SDK_CameraParamEx;
...
//@@###########################@@B 平台
typedef struct{
	char Code[256];				//设备地址编码
	char Name[256];				//设备名称
	double Longitude;			//GPS经度
	double latitude;			//GPS纬度
	char Supplier[256];			//供应商名称
	char APN[256];				//APN名称
	char SIMCARDNo[32];			//流量卡号
	unsigned int RSSI;			//信号强度
	char IPADDR[24];			//IP地址
	unsigned int MonConsum;		//本月已用
	char SIMStatus[256];		//流量卡状态
}SDK_XM_firm_sim_info_t;
	
typedef struct{
	char Code[256];				//电源设备地址编码
	char Name[256];				//电源设备名称
	char SolarPanelStatus[256];	//太阳能板状态
...
typedef struct
{
	SDK_XM_firm_battery_info_t Dev[MAX_RS485_NUMBER];
}SDK_XM_All_battery_info_t;

typedef struct{
	char ServerID[64];				////平台编码
	char ServerIP[24];				//平台IP
	int  ServerPort;				//平台端口号
	char DeviceID[64];				//设备编码
	char DeviceIP[24];				//设备IP
	char ChannelID[32][64];			//前端设备支持的通道信息
	char DevicePassword[64];		//设备端密码
	int ActChannelNum;				//实际通道个数
...
//服务器断开回调原形
typedef void (CALL_METHOD *fDisConnect)(long lLoginID, char *pchDVRIP, long nDVRPort, USERTYPE dwUser);
//原始数据回调原形
typedef int(CALL_METHOD *fRealDataCallBack) (long lRealHandle, long dwDataType, unsigned char *pBuffer,long lbufsize,USERTYPE dwUser);
typedef int(CALL_METHOD *fRealDataCallBack_V2) (long lRealHandle, const PACKET_INFO_EX *pFrame, USERTYPE dwUser);

// 回放录像进度回调原形
typedef void(CALL_METHOD *fDownLoadPosCallBack) (long lPlayHandle, long lTotalSize, long lDownLoadSize, USERTYPE dwUser);
...
/*
	...
	3.定制图片数据推送----nType=PUSH_TYPE
	4.通用人脸图片数据推送----nType=PUSH_TYPE_NORMAL_FACE	
*/
typedef bool (CALL_METHOD *fMessCallBack)(long lLoginID, unsigned char *pBuf, unsigned long dwBufLen, USERTYPE dwUser, int nType, void * pDataInfo);
...
typedef void (CALL_METHOD *pfAudioDataCallBack)(long lVoiceHandle, char *pDataBuf,
                                                long dwBufSize, char byAudioFlag, USERTYPE dwUser);


//本地播放结束回调原形
typedef void (CALL_METHOD * fLocalPlayFileCallBack)(long lPlayHand, USERTYPE nUser);

//信息帧回调设置
typedef void (CALL_METHOD  *InfoFramCallBack)(long lPlayHand, long nType, LPCSTR pBuf,long nSize, USERTYPE nUser);

//子连接断线回调
typedef void (CALL_METHOD *fSubDisConnectCallBack)(long lLoginID, SubConnType type, long nChannel, USERTYPE dwUser);

//发送文件数据回调
/*
	nResult = 1表示发送完毕
	nResult = 2表示发送设备返回失败---具体错误：nError(101:未知错误;109：查找失败，没有找到对应文件;603:需要重启设备才生效)
...
//nChannel请求的通道号
//nType返回的请求类型
//nRet == 100成功，其它皆为失败，返回的是设备的错误码，具体可以找设备了解
typedef void (CALL_METHOD *fExportFileCallBack)(long lLoginID, int nChannel, int nType, int nRet, void * dwUser);

#ifdef WIN32
//RigisterDraw回调原形
typedef void (CALL_METHOD * fPlayDrawCallBack)(long lPlayHand,HDC hDc,USERTYPE nUser);
#else//linux没用到这个回调
...
/*
	...
	返回值:
		...
		<=0:失败
*/
H264_DVR_API long CALL_METHOD H264_DVR_Init(fDisConnect cbDisConnect, USERTYPE dwUser);
...
/*
	...
	返回值:
		...
		false:失败
*/
H264_DVR_API bool CALL_METHOD H264_DVR_SetDVRMessCallBack(fMessCallBack cbAlarmcallback, USERTYPE lUser);
...
/*
	...
	返回值:
		...
		false:失败
*/
H264_DVR_API bool CALL_METHOD H264_DVR_SetRealDataCallBack(long lRealHandle,fRealDataCallBack cbRealData, USERTYPE dwUser);

/*
	描述:数据经过码流分析过，带有帧的具体信息,其他与H264_DVR_SetRealDataCallBack相同
*/
H264_DVR_API bool CALL_METHOD H264_DVR_SetRealDataCallBack_V2(long lRealHandle,fRealDataCallBack_V2 cbRealData, USERTYPE dwUser);
...
/*
	...
	返回值:
		...
		false:失败
*/
H264_DVR_API bool CALL_METHOD H264_DVR_DelRealDataCallBack(long lRealHandle,fRealDataCallBack cbRealData, USERTYPE dwUser);
...
/*
	...
	返回值:
		...
		false:失败
*/
H264_DVR_API bool CALL_METHOD H264_DVR_DelRealDataCallBack_V2(long lRealHandle,fRealDataCallBack_V2 cbRealData, USERTYPE dwUser);
...
/*
	...
	返回值:
		...
		<=0:	   失败
*/
H264_DVR_API long CALL_METHOD H264_DVR_PlayBackByName(long lLoginID, H264_DVR_FILE_DATA *sPlayBackFile, fDownLoadPosCallBack cbDownLoadPos, fRealDataCallBack fDownLoadDataCallBack, USERTYPE dwDataUser);
...
/*
	...
	返回值:
		...
		<=0:	   失败
*/
H264_DVR_API long CALL_METHOD H264_DVR_PlayBackByName_V2(long lLoginID, H264_DVR_FILE_DATA *sPlayBackFile, fDownLoadPosCallBack cbDownLoadPos, fRealDataCallBack_V2 fDownLoadDataCallBack, USERTYPE dwDataUser);
...
/*
	...
	返回值:
		...
		<=0:	   失败
*/
H264_DVR_API long CALL_METHOD H264_DVR_PlayBackByTime(long lLoginID, H264_DVR_FINDINFO* lpFindInfo, fDownLoadPosCallBack cbDownLoadPos, fRealDataCallBack fDownLoadDataCallBack, USERTYPE dwDataUser);
...
*/
H264_DVR_API long CALL_METHOD H264_DVR_PlayBackByTimeEx(long lLoginID, H264_DVR_FINDINFO* lpFindInfo,fRealDataCallBack fDownLoadDataCallBack, USERTYPE dwDataUser,
											fDownLoadPosCallBack cbDownLoadPos, USERTYPE dwPosUser);
/*
	描述:fDownLoadDataCallBack数据回调出来的数据的码流分析过，其他与H264_DVR_PlayBackByTimeEx相同
*/
H264_DVR_API long CALL_METHOD H264_DVR_PlayBackByTime_V2(long lLoginID, H264_DVR_FINDINFO* lpFindInfo,fRealDataCallBack_V2 fDownLoadDataCallBack, USERTYPE dwDataUser,
											fDownLoadPosCallBack cbDownLoadPos, USERTYPE dwPosUser);
...
H264_DVR_API long CALL_METHOD H264_DVR_GetFileByName(long lLoginID,H264_DVR_FILE_DATA *sPlayBackFile,char *sSavedFileName,
                                                     fDownLoadPosCallBack cbDownLoadPos DEF_0_PARAM, USERTYPE dwDataUser DEF_0_PARAM,fRealDataCallBack fDownLoadDataCallBack DEF_0_PARAM);
...
H264_DVR_API long CALL_METHOD H264_DVR_GetFileByName_V2(long lLoginID,H264_DVR_FILE_DATA *sPlayBackFile,char *sSavedFileName,
													 fDownLoadPosCallBack cbDownLoadPos DEF_0_PARAM, USERTYPE dwDataUser DEF_0_PARAM,fRealDataCallBack_V2 fDownLoadDataCallBack_V2 DEF_0_PARAM);
...
/*
	...
	返回值:
		...
		=0:		失败
*/
H264_DVR_API long CALL_METHOD H264_DVR_StartVoiceCom_MR(long lLoginID, pfAudioDataCallBack pVcb, USERTYPE dwDataUser);
...
/*
	...
	返回值:
		...
		=0:		失败
*/
H264_DVR_API long CALL_METHOD H264_DVR_StartVoiceCom_NVR_IPC(long lLoginID, int nChannel, pfAudioDataCallBack pVcb, USERTYPE dwDataUser);
...
/*
	...
	返回值:
		...
		false:失败
*/
H264_DVR_API bool CALL_METHOD H264_DVR_StartAlarmCenterListen(int nPort, fMessCallBack cbAlarmCenter, USERTYPE dwDataUser);
...
/*
	...
	返回值:本地播放句柄
		...
		=0:	   失败
*/
H264_DVR_API long CALL_METHOD H264_DVR_StartLocalPlay(char*pFileName,void* hWnd,fPlayDrawCallBack drawCallBack=0,USERTYPE user=0);
...
/*
	...
	返回值:
		...
		false:失败
*/
H264_DVR_API bool CALL_METHOD H264_DVR_SetFileEndCallBack(long lPlayHandle,fLocalPlayFileCallBack callBack,USERTYPE user);
...
/*
	...
	返回值:
		...
		false:失败
*/
H264_DVR_API bool CALL_METHOD H264_DVR_SetInfoFrameCallBack(long lPlayHandle,InfoFramCallBack callback,USERTYPE user);
...
/*
	...
	返回值:
		...
		false:失败
*/
H264_DVR_API bool CALL_METHOD H264_DVR_StartActiveRigister(int nPort, fMessCallBack cbFunc, USERTYPE dwDataUser);
...
/*
	...
	返回值:
		...
		=0:失败
*/
H264_DVR_API long CALL_METHOD H264_DVR_SetSubDisconnectCallBack(fSubDisConnectCallBack callBack,USERTYPE userData);
...
/*
	...
	参数:
		...
		MsgID[in]:			 消息ID--1042
		nChannelNO[in]:		 通道号,-1:得到全通道配置,0-n:得到单个通道的配置,从0开始
		lpInBuffer[in]:		 请求的json字符串
		lpInBufferlen[in]:	 请求的json字符串大小
		lpOutBuffer[out]:	 接收json缓冲
		dwOutBufferSize[in]: 接收缓冲大小
		lpBytesReturned[out]:得到的缓冲大小(字节)
		waittime[in]:		 等待时间
